...
class MessageSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if _unicity.get(_prefix + '__' + 'from_user', None):
            ...
            if user_data:
                print(user_data)
                __user = AndruserSerializer(data=user_data)
        ...
        if _unicity.get(_prefix + '__' + 'pinned_message', None):
            validated_data['pinned_message'] = Message.objects.get(
                message_id=_unicity[_prefix + '__' + 'pinned_message'])

        if _unicity.get(_prefix + '__' + 'reply_to_message', None):
            validated_data['reply_to_message'] = Message.objects.get(
                message_id=_unicity[_prefix + '__' + 'reply_to_message'])
        ...
        if message_auto_delete_timer_changed_data:
            message_auto_delete_timer_changed = MessageAutoDeleteTimerChangedSerializer(
                data=message_auto_delete_timer_changed_data)
        ...
        if voice_chat_participants_invited_data:
            voice_chat_participants_invited = VoiceChatParticipantsInvitedSerializer(
                data=voice_chat_participants_invited_data)
